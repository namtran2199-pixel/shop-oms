...
function PeriodValuePicker({
  ...
  const initialYear =
    period === "month" ? parseMonthValue(safeValue).year : Number(safeValue) || today.year;
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {data.cards.map((card) => (
          <Card key={card.label} className="p-5">
            ...
            <p className="mt-3 text-3xl font-semibold text-near-black-ink">{card.value}</p>
          </Card>
        ))}
      </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <PickerShell label="Mốc thời gian">
              <PeriodValuePicker period={period} value={periodValue} onChange={setPeriodValue} />
            </PickerShell>
          </div>
        </div>
      </Card>
          ...
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-secondary-neutral-gray">
                Kỳ đang xem: {data.chartFilter.periodLabel} • {data.charts.periodRevenue} •{" "}
                {data.charts.periodOrderCount} đơn
              </p>
            </div>
          </div>
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <Card className="h-fit p-6">
          ...
          <div className="mt-4 h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={data.charts.topProductsSeries}
                layout="vertical"
